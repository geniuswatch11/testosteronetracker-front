...
import type { HeartRateDataPoint, ApiErrorResponse } from "@/lib/types/api"

interface HeartRateRestingBarChartProps {
  period: string
}

export function HeartRateRestingBarChart({ period }: HeartRateRestingBarChartProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await metricsApi.getHeartRateStats()

        if ("error" in response && response.error !== "") {
          const errorResponse = response as ApiErrorResponse
          setError(errorResponse.message || "Error al cargar datos")
        } else {
          const newData = (response.data || []) as HeartRateDataPoint[]
          if (newData.length > 0) {
            setData(newData)
          }
        }
    ...
    return (
      <div className="bg-neutral-600 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-white text-lg font-semibold">heartrate Resting</h3>
          <InfoTooltip description="Your resting heart rate is a key indicator of cardiovascular fitness. Lower values typically indicate better heart health." />
        </div>
