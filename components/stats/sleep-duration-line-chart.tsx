...
import type { SleepDurationDataPoint, ApiErrorResponse } from "@/lib/types/api"

interface SleepDurationLineChartProps {
  period: string
}

export function SleepDurationLineChart({ period }: SleepDurationLineChartProps) {
  const [data, setData] = useState<SleepDurationDataPoint[]>([])
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if ("error" in response && response.error !== "") {
          ...
          const newData = (response.data || []) as SleepDurationDataPoint[]
          if (newData.length > 0) {
            setData(newData)
          }
        }
    ...
    return (
      <div className="bg-neutral-600 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-white text-lg font-semibold">Sleep Duration</h3>
          <InfoTooltip description="Track your total sleep duration including deep sleep and REM phases. Aim for 7-9 hours of quality sleep per night." />
        </div>
