...
export const getTestosteroneStatus = (
  value: number,
  birthDate: string
): string => {
...
export default function TestosteroneTable({
  birthDate,
  testosteroneValue,
  isLoading,
}: TestosteroneTableProps) {
  const { t } = useLanguage();
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        className="rounded-xl bg-gray-50 dark:bg-[#1a1a1a] p-3 shadow-sm"
                ...
                <td
                  ...
                  className={`px-1 py-1 text-xs text-center border-t border-gray-200 dark:border-gray-700 transition-colors ${
                    isActive(range.label)
                      ? `${getColorClass(range.label)} font-bold`
                      : ""
      ...
      <p className="text-xs text-muted-foreground text-center px-4">
        {t("charts.testosteroneTableNote")}
      </p>
    </div>
  );
}
