...
export function calculateAverage(data: HealthData[]): {
  ...
  const sum = data.reduce(
    (acc, point) => ({
      testosterone: acc.testosterone + (point.testosterone ?? 0),
      resting_hr: acc.resting_hr + (point.resting_hr ?? 0),
      hrv: acc.hrv + (point.hrv ?? 0),
      sleep_hours: acc.sleep_hours + (point.sleep_hours ?? 0),
      sleep_efficiency: acc.sleep_efficiency + (point.sleep_efficiency ?? 0),
      respiratory_rate: acc.respiratory_rate + (point.respiratory_rate ?? 0),
      spo2: acc.spo2 + (point.spo2 ?? 0),
      calories_total: acc.calories_total + (point.calories_total ?? 0),
      active_calories: acc.active_calories + (point.active_calories ?? 0),
    }),
    {
      testosterone: 0,
      resting_hr: 0,
